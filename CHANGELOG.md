...
### Changed
- `before_last()` builds its result in one pass instead of counting, taking
  and re-joining the split parts (~20-30% faster on JavaScript for long
  inputs with many matches; unchanged on Erlang).
